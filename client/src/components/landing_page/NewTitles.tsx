...
function NewTitleItem({ bookImage, bookDetails }: { bookImage: string, bookDetails?: { title?: string, author?: string, rating?: number, year?:number } }) {
    ...
    if (bookDetails?.title !== undefined) {
        return (
            <div className="hover:scale-105 transition duration-300 w-1/4 max-[900px]:mt-5 max-[900px]:w-[31%] max-sm:w-full h-90 max-[900px]:h-fit flex relative rounded-3xl flex-col backdrop-blur-2xl overflow-hidden px-5 py-6 border bg-[#48576019] border-gray-800">
                <span className="flex w-fit mx-auto h-7/11 max-[900px]:h-35"><img src={bookImage} className="h-full w-auto" alt="Addict" /></span>
                <h3 className="text-lg max-[900px]:text-sm text-gray-400 mt-5">{truncate(bookTitle, 24)}</h3>
