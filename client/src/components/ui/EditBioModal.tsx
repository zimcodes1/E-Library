...
function EditBioModal({
	...
	const handleSave = async () => {
		if (bioEdit && bioEdit !== userBio) {
			...
			setEditMessage('No changes made to the bio.');
	...
	return (
		<div className="w-100 h-fit max-sm:w-9/10 bg-[#1a1b2e] border border-purple-500/30 text-gray-200 rounded-2xl shadow-2xl py-8 px-6 flex flex-col relative">
            {(editMessage)? <Message type="fail" text={editMessage} /> : null}
			<i
				onClick={onClose}
			...
			<textarea
				...
				name="userBio"
				id="userBio"
				className="bg-purple-200/10 resize-none my-4 outline-0 ring-0 rounded-2xl p-4 text-sm h-25"
