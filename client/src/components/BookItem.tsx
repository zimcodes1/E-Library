...
function BookItem({bookImage, bookDetails, customWidth, bookId }: { bookImage: string, bookDetails?: { title?: string, author?: string, rating?: number, year?:number }, customWidth?:string, bookId?: number }) {
    let bookTitle = bookDetails?.title
    if (bookDetails?.title !== undefined) {
        return (
            <Link to={`/bookdetails/${bookId}`} className={`w-27 max-sm:${customWidth || 'w-[32.5%]'} my-2 max-sm:my-1 shrink-0 h-40 flex flex-col justify-start items-start p-2 bg-[#31303e6d] border border-gray-700 rounded-2xl cursor-pointer overflow-hidden hover:border-purple-500 hover:shadow-lg hover:shadow-purple-500/20 transition-all duration-300 group`} title={bookTitle}>
        ...
        <Link to={`/bookdetails/${bookId}`} className={`w-27 shrink-0 h-full flex justify-center items-center p-2 bg-[#31303e] border border-gray-700 rounded-2xl cursor-pointer overflow-hidden hover:border-purple-500 hover:shadow-lg hover:shadow-purple-500/20 transition-all duration-300 group`}>
