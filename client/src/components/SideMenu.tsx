...
function SideMenu() {
    ...
    const navLinkStyles = ({ isActive }: { isActive: string | any }) =>
        `w-full text-sm h-12 flex justify-start max-[900px]:justify-center max-[900px]:mt-3 max-sm:mt-0 items-center cursor-pointer px-2 rounded-md max-sm:rounded-full transition duration-300 ${isActive
            ? "bg-[#48576050] text-white font-medium" // Active styles
            : "text-gray-300 hover:bg-[#48576030]"    // Inactive/Hover styles
        }`;

    return (
        <div className="w-1/7 max-[900px]:w-1/9 max-sm:w-[95%] max-sm:h-18 backdrop-blur-2xl max-sm:rounded-4xl
         max-sm:flex-row max-sm:fixed max-sm:z-50 max-sm:bg-transparent
